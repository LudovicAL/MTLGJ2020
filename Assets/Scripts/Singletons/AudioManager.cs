...
public class AudioManager : MonoBehaviour {
	...
	public int maxNbAudioSources = 15;
	[Range(0.0f, 1.0f)] public float themeSongVolume = 0.3f;
	...
	public AudioSource PlayClip(AudioClip clip) {
		return PlayClip(clip, false, 0.0f, 1.0f, 1.0f);
	}
	...
	public AudioSource PlayClip(AudioClip clip, bool loop) {
		return PlayClip(clip, loop, 0.0f, 1.0f, 1.0f);
	}
	...
	public AudioSource PlayClip(AudioClip clip, float fadeInLength) {
		return PlayClip(clip, false, fadeInLength, 1.0f, 1.0f);
	}
	...
	/// <summary>
	/// Pauses all AudioSources with a fade out
	/// </summary>
	...
	/// <summary>
	/// Resumes an Audiosource
	/// </summary>
	public void ResumeAudioSource(AudioSource audioSource) {
		ResumeAudioSource(audioSource, 0.0f, 1.0f);
	}

	/// <summary>
	/// Resumes all AudioSources with a fade in
	/// </summary>
	public void ResumeAllAudioSource(float fadeInLength) {
		foreach (AudioSource audioSource in playingAudioSourceList) {
			ResumeAudioSource(audioSource, fadeInLength, 1.0f);
		}
	}

	/// <summary>
	/// Resumes an AudioSource with a fadein
	/// </summary>
	public void ResumeAudioSource(AudioSource audioSource, float fadeInLength) {
		ResumeAudioSource(audioSource, fadeInLength, 1.0f);
	}

	/// <summary>
	/// Resumes an AudioSource with a fadein and a desired volume
	/// </summary>
	public void ResumeAudioSource(AudioSource audioSource, float fadeInLength, float desiredVolume) {
		if (audioSource) {
			audioSource.Play();
			if (fadeInLength > 0.0f) {
				StartCoroutine(AudioSourceFadeIn(audioSource, fadeInLength, desiredVolume));
			...
			Debug.Log("You passed a null AudioSource. What is wrong with you?");
		}
	}
